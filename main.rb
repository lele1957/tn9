...
class Menu
  ...
  def delete_station_from_route
    ...
    puts 'Введите номер станции из списка'
    @routes_list[delete_from_route - 1].stations.each_with_index do |station, number|
      puts "#{number + 1} - #{station.name}"
    end
  ...
  def move_train
    ...
    @trains_list[train_to_move - 1].station_forward if %w[вперед forward].include?(direction)
    @trains_list[train_to_move - 1].station_backward if %w[назад backward].include?(direction)
  ...
  def view_station_list
    ...
    @stations_list.each_with_index do |station, number|
      puts "#{number + 1} - #{station.name}, поезда: #{station.trains}"
    end
  end
